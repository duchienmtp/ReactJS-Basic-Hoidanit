    ...
    if (res && res.item) {
      ...
      if (raw && raw.length) {
        raw.map((item, index) => {
          ...
          object.publishedAt = moment(item.snippet.publishedAt).format(
            "DD/MM/YYYY"
          );
          result.push(object);
        });
        ...
        videos.map((item, index) => {
          return (
            <div className="yt-result" key={item.id}>
              ...
              <div className="right">
                <div className="title">
                  Tơng Lai Của Dev React.JS | Nên Học Gì Để Không Thất Nghiệp
                  Khi React Thay Đổi ?ư
                </div>
                <div className="created-at">
                  Created At:{" "}
                  {moment("2023-04-23T10:00:10Z").format("DD-MM-YYYY HH:mm:ss")}
                </div>
                <div className="author">Hỏi Dân IT</div>
                <div className="description">
                  Vào tháng 3, 2023, React chính thức có ngôi nhà mới với địa
                  chỉ https://react.dev, nhà cũ reactjs.org đã được đổi thành
                  ...
                </div>
              </div>
            </div>
